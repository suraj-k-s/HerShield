<%-- 
    Document   : ComplaintUploadAction
    Created on : 1 Mar, 2024, 10:56:25 AM
    Author     : hp
--%>
    ...
    if (isMultipart) {
        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("filecomplaint"))//write the name of image here it is img_product
                {
                    ...
                    photo = "complaintfile_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
        ...
        String InsQry = "INSERT INTO tbl_complaint( complaint_title,complaint_details,complaint_file,complaint_date,user_id,complaint_latitude,complaint_longitude)"
                + "values('" + value[0] + "','" + value[1] + "','" + photo + "',curdate(),'" + session.getAttribute("Uid") + "','" + value[2] + "','" + value[3] + "')";

        DB.MailClass m = new DB.MailClass();
        String subject = "Alert";
        String text = "";
        String mailid[] = {session.getAttribute("gemail").toString()};
...
<script type="text/javascript">
    alert("Complaint Registration Completed");
    setTimeout(function() {
        window.location.href = '../../User/SubmitComplaint.jsp'
