<%-- 
    Document   : AdvocateUploadAction
    Created on : 22 Feb, 2024, 6:01:35 PM
    Author     : hp
--%>
...
//checking if request cotains multipart data
            ...
            if (isMultipart) {
                ...
                while (itr.hasNext())//checking if there is any value in the position like rs.next()
                {
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (field_name.equals("filephoto"))//write the name of image here it is img_product
                        {
                            ...
                            photo = "AdvovatePhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("photofile"))//write the name of image here it is img_product
                        {
                            String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            ...
                            proof = "AdvovateProof_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
                ...
                if (value[6].equals(value[6])) {//Strinh str1="insert into tbl_user ()";//inserting file
                    String InsQry = "INSERT INTO tbl_advocate( advocate_name, advocate_email,advocate_address,place_id,specialization_id, advocate_photo,advocate_proof, advocate_password)"
                            + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[4] + "','" + value[5]  + "','" + photo  + "','"+ proof + "','"+ value[6] + "')";
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Guest/Advocate.jsp'
